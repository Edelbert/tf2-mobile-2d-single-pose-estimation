    ...
    def __init__(self, block_num=3, channel=48, channel_padding=1, name_prefix=""):
        ...
        self.downsample_a = tf.keras.models.Sequential([
            tf.keras.layers.DepthwiseConv2D(kernel_size=3, strides=(
                2, 2), padding='same', activation=None, kernel_regularizer=self.l2_regularizer_00004, name=name_prefix+"downsample_a_depthwise"),
        ...
        for i in range(block_num):
            self.conv.append(tf.keras.models.Sequential([
                tf.keras.layers.DepthwiseConv2D(
                    kernel_size=3, padding='same', activation=None, kernel_regularizer=self.l2_regularizer_00004, name=name_prefix+"conv_block_{}".format(i+1)),
                tf.keras.layers.Conv2D(
